...
class Treatment:
    ...
    def init_check_structure(self):
        ...
        if len(self.structure_paths) == 0:
            raise rtex.InitError(
                f"Error, no structure for patient {self.patient_id} , treatment {self.treatment_id}"
            )
        elif len(self.structure_paths) > 1:
            raise rtex.InitError(
                f"Error, more than one structure for patient {self.patient_id} , treatment {self.treatment_id}"
            )
    ...
    def init_check_cts(self):
        ...
        if len(self.ct_paths) == 0:
            raise rtex.InitError(
                f"Error, no cts for patient {self.patient_id} , treatment {self.treatment_id}"
            )
        else:
            return True
    ...
    def init_check(self):
        ...
        if len(self.sum_dose_paths) == 0:
            if len(self.dose_paths) == 1 and self.main_dose_scale_factor == 1:
                ...
                try:
                    self.sum_dose_path, self.dose = rtsum.create_sum_dose_file(
                        self.dose_paths,
                        self.patient_id,
                        self.treatment_id,
                        self.get_plans(),
                        self.main_reference_dose,
                        self.main_dose_scale_factor,
                        self.boost_reference_dose,
                        self.boost_dose_scale_factor,
                    )
                ...
                if self.sum_dose_path is None:
                    ...
                    raise rtex.InitError(
                        f"Failed for patinet: {self.patient_id} Dose summation failed"
                    )
                else:
        ...
        # If the doses have already been summed
            ...
            raise rtex.InitError(
                f"Failed for patinet: {self.patient_id} More than one sum_dose for patienttreatment {self.treatment_id}"
            )

        return True

    def init_check_no_dose(self):
        """Raise exception if initation checks fail"""
        try:
    ...
    def get_ct_image_data(self):
        ...
        if self.image_data is None:
            try:
                self.image_data = dicomparser.DicomParser(
                    self.ct_paths[0]
                ).GetImageData()
            except IndexError:
                raise rtex.NoCtsError(
                    f"{self.patient_id} has no cts. get_ct_image_data failed"
                )
    ...
    def right_left_dose_grid_ratio(self):
        ...
        try:
            ct_mid_position_patient_coordinates = self.get_ct_image_data()["position"][
                0
            ] + (
                self.get_ct_image_data()["columns"]
                * self.get_ct_image_data()["pixelspacing"][0]
                / 2
            )
        except rtex.NoCtsError as e:
            raise e

        dose_grid_ct_mid_position_patient_coordinates = (
            ct_mid_position_patient_coordinates - rt_dose.ds.ImagePositionPatient[0]
        )

        dose_grid_mid_pixel = int(
            abs(
                round(
                    dose_grid_ct_mid_position_patient_coordinates
                    / rt_dose.ds.PixelSpacing[0],
                    0,
                )
            )
        )
        ...
        if rt_dose is None:
            ...
            for index, z in enumerate(planes):
                ...
                try:
                    right_integral_dose = right_integral_dose + np.percentile(
                        r_grid, 95
                    )
                except IndexError:
                    continue
    ...
    def get_augmented_structures_for_patient(self, structure_collection_id=None):
        ...
        with rtdb.DatabaseCall() as db:
            astructs = db.get_augmented_structure_paths_for_patient(
                patient_id=self.patient_id,
                structure_collection_id=structure_collection_id,
            )
    ...
    def get_structure_information(self, standard_name):
        ...
        with rtdb.DatabaseCall() as db:
            synonym_list = db.get_synonyms_from_standard_name(
                standard_name, self.treatment_collection_id
            )
        ...
        for index, roi in roi_names.items():
            if (roi["name"].lower() in synonym_list) and not (roi["empty"]):
                ...
                for key in self.plane_list:
                    set_with_most_pts = (
                        list()
                    )  ##In some cases, two coordinatesets are present for the structures. This removes the coordinateset, with the fewest coordinates. This is not tested on other structures than LADCA! So it's not necessaily a correct assumption for all structures
                    data_pts = planes[key]
                    if len(data_pts) > 1:
...
class Roi:
    ...
    def find_synonyms(self):
        ...
        with rtdb.DatabaseCall() as db:
            row_data = db.get_synonyms_from_standard_name(
                self.standard_name,
                self.treatment.treatment_collection_id,
                priority=True,
            )
        ...
        for key, structure in structures_in_file.items():
            if any(
                d["name"] == structure["name"].lower() for d in synonyms_standard
            ) and not (structure["empty"]):
                self.synonyms_found.append(
                    {
                        "name": structure["name"],
                        "structure_id": structure["id"],
                        "priority_count": next(
                            (
                                item.get("priority_count")
                                for item in synonyms_standard
                                if item["name"].lower() == structure["name"].lower()
                            ),
                            False,
                        ),
                        "laterality": next(
        ...
        self.synonyms_found = sorted(
            self.synonyms_found, key=lambda i: (i["laterality"], -i["priority_count"])
        )
    ...
    def calculate_dvh(self, structure_id):
        ...
        calcdvh = dvhcalc.get_dvh(
            self.treatment.structure_path, self.treatment.dose_path, structure_id
        )
        ...
        if self.priority_synonym is None:
            ...
            for structure in synonyms:
                if structure["laterality"] == "i":  # ipsilateral"
                    ...
                    break
                if (
                    structure["laterality"] == "l"
                    and self.treatment.get_latterality() == "left"
                ):
                    priority_structure = structure
                    break
                elif (
                    structure["laterality"] == "r"
                    and self.treatment.get_latterality() == "right"
                ):
                    priority_structure = structure
                    break
    ...
    def get_value_for_dvh_data_point(self, dvh_data_point, prioritize_roi_name=False):
        ...
        if dvh_point in ["volume", "mean", "max", "min"]:
            ...
            if output_type == "rel":
                try:
                    dvh_data_point.value = getattr(
                        self.dvh_priority.relative_volume, dvh_point
                    ).value
                except:
                    dvh_data_point.value = None
            else:
    ...
    def dvh_points_from_roi(self, data_dict, dvh_points):
        ...
        for dvh_point_input in dvh_points:
            ...
            if dvh_point in ["volume", "mean"]:
                try:
                    data_dict[f"{roi_name}_{dvh_point}"] = getattr(
                        self.dvh_priority, dvh_point
                    )
                except:
