...
def create_sum_dose_file(dose_paths, patient_id, treatment_id, plans, main_reference_dose, 
    ...
    if len(plans) == 1:
        # primary plan scaled, SIB plan scaled, primary plan (multi doses) summed, primary (multi doses) summed + scaled
        ...
        if len(plans) > len(dose_paths):    
            ...
            raise rtex.SumDoseError(f'Failed for patinet: {patient_id} More plans than dose files. Plans: {plan_names}')
            

        #primary (mulit plans) summed + scaled, #primary (mulit plans) summed
        if boost_reference_dose == 0:
            ...
            # primary + boost summed, primary (scaled) + boost summed, primary (scaled) + boost summed  

                # Sum one main dose and one boost dose (We cannot handle plans with multiple doses when scaling boost/main)
                if len(dose_paths) != 2:
                    raise rtex.SumDoseError(
                        f'Failed for patinet: {patient_id}' +
                        f'Number of dose files is not 2, but {len(dose_paths)}' + 
                        f'boost reference_dose = {print(boost_reference_dose)}' +
                        f'main reference_dose = {print(main_reference_dose)}'
                     )
                ...
                for plan in plans: 
                    ...
                    fractions_plan_uid.append ([no_fractions[0], plan.GetSOPInstanceUID()])

                fractions_dose_paths = list()
                for dose_path in dose_paths:
                    ref_plan_uid = dicomparser.DicomParser(dose_path).GetReferencedRTPlan()  
                ...
                if main_reference_dose == 50:
                    ...
                    raise rtex.SumDoseError(f'Failed for patient: {patient_id} main_reference dose is not standard but {main_reference_dose}')
                ...
                if boost_reference_dose == 16:
                    ...
                    raise rtex.SumDoseError(f'Failed for patient: {patient_id} boost_reference dose is not standard but {boost_reference_dose}')
                ...
                if main_dose_scale_factor > max_scale:
                    raise rtex.SumDoseError(f'Failed for patinet: {patient_id}. Unable to identify main plan. Main dose scale factor is {main_dose_scale_factor}')   
                else:
                    dose_path_scale_pairs.append([fractions_dose_paths[0][1], boost_dose_scale_factor])
                    dose_path_scale_pairs.append([fractions_dose_paths[1][1], main_dose_scale_factor])
    ...
    if os.path.exists(local_folder_path):
        ...
        try:
            ...
            print(f"{patient_id} - Creation of the directory {local_folder_path} failed")
    ...
    with open(f'{local_folder_path}/log_treatment_{treatment_id}.txt', 'w') as f:
        for item in log:
            f.write("%s\n" % item)
    
    if write_line_in_db:
        row_data = [treatment_id, 'sum_dose', sum_dose_path, summed_dose_dicom_file.GetSOPInstanceUID()] 
        with rtdb.DatabaseCall() as db:
            db.insert_row_in_table('dicom_files', ['treatment_id', 'file_type', 'file_path, file_uid'], row_data)
    ...
    for pair in dose_path_scale_pairs: 
        ...
        if dose_grid_sum is None:
            try:
                ...
                raise rtex.SumDoseError(f'Failed for patinet: {patient_id}. Error, failed to init DoseGrid in sum_doses for, file: {path}')
            ...
            #The lowest doses are dose per beam, and they should be more than 1 Gy max
            if (max_dose > 1): # and (getattr(dose_grid.ds, 'DoseSummationType') != 'CONTROL_POINT'):
