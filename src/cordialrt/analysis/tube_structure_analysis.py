...
class tube_structure_analysis:
    ...
    def plot_planes(self, centroid_value_x, centroid_value_y, z_coordinate):
        ...
        scale = self.SplineTube(
            centroid_value_x,
            centroid_value_y,
            z_coordinate,
            n=len(centroid_value_x) + 50,
        )

        return (scale, structure_length)

    def SplineTube(
        self, centroid_value_x, centroid_value_y, z_coordinate, n, degree=2, **kwargs
    ):
        """This function returns the scaling needed to get the 'true' width of the structure"""
        assert n >= 3
        scale_list = list()
        (tck, u), fp, ier, msg = splprep(
            [centroid_value_x, centroid_value_y, z_coordinate],
            s=0,
            k=2,
            full_output=True,
        )
    ...
    def longest_distance(self, x, y):
        ...
        dist = math.sqrt(
            (x_cands[1] - x_cands[0]) ** 2 + (y_cands[1] - y_cands[0]) ** 2
        )
    ...
    def get_width(self, plot_structure=None):
        ...
        Matrix = sorted(
            zip(centroid_value_x, centroid_value_y, z_coordinate),
        ...
        for i in range(len(z_coordinate) - 1):
            distance_outlier_test = math.sqrt(
                (centroid_value_x[i + 1] - centroid_value_x[i]) ** 2
                + (centroid_value_y[i + 1] - centroid_value_y[i]) ** 2
                + (z_coordinate[i + 1] - z_coordinate[i]) ** 2
            )
        ...
        Matrix_2 = sorted(
            zip(
                centroid_value_x_without_outliers,
                centroid_value_y_without_outliers,
                z_coordinate_without_outliers,
            ),
            key=operator.itemgetter(2),
        )
        ...
        if len(centroid_value_x) <= 3:
            ...
            scale, self.structure_length = self.plot_planes(
                centroid_value_x, centroid_value_y, z_coordinate
            )
    ...
    def get_cranial_caudal_extension(self):
        ...
        if len(self.treatment.plane_list) <= 3:
            ...
            self.cranial_caudal_extension = (
                len(self.treatment.plane_list) - 1
            ) * self.treatment.slice_thickness  ## mm
